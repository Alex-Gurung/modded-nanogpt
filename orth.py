...
class Block(nn.Module):
    def __init__(self, dim: int, head_dim: int, num_heads: int, layer_idx: int):
        ...
        # You can optionally vary this per-layer if you want, but a single global
        # constant is a good starting point and adds zero parameters.
        self.ortho_strength_attn = 1.0  # try e.g. 0.7–1.0
        self.ortho_strength_mlp  = 1.0  # try e.g. 0.7–1.0

    def forward(self, x: Tensor, x0: Tensor, lambdas: Tensor, attn_args: AttnArgs):
        # Existing exponential residual mixing with x0
        x = lambdas[0] * x + lambdas[1] * x0

        # Attention block with orthogonal residual update
        if self.attn is not None:
            attn_in = norm(x)
            attn_out = self.attn(attn_in, attn_args)   # [B, T, D]
            x = orthogonal_residual(
                x,
                attn_out,
                eps=1e-6,
                strength=self.ortho_strength_attn,
            )

        # MLP block with orthogonal residual update
        if self.mlp is not None:
            mlp_in = norm(x)
            mlp_out = self.mlp(mlp_in)                # [B, T, D]
            x = orthogonal_residual(
                x,
                mlp_out,
                eps=1e-6,
                strength=self.ortho_strength_mlp,
            )

        return x

# ----------------------------------------------------------------------------- 
# Orthogonal residual update (feature-wise), adapted from
# "Orthogonal Residual Updates" (2025). :contentReference[oaicite:5]{index=5}
#
# x, f: [B, T, D] tensors (residual stream and block output).
# We remove the component of f that is parallel to x, token-wise:
#   f_perp = f - s * x, where
#   s = <x, f> / (||x||^2 + eps), with inner product over feature dim.

def orthogonal_residual(x: Tensor, f: Tensor, eps: float = 1e-6, strength: float = 1.0) -> Tensor:
    """
    Apply an orthogonal residual update:
        x_new = x + f_perp
    where f_perp is the component of f orthogonal to x, computed per (batch, token).

    strength ∈ [0, 1]: 1.0 = full orthogonal update, 0.0 = standard residual.
    """
    if strength == 0.0:
        return x + f

    # x, f: [B, T, D]
    # Inner product along feature dimension
    # sm: [B, T, 1] = <x, f>
    sm = (x * f).sum(dim=-1, keepdim=True)
    # sd: [B, T, 1] = ||x||^2
    sd = (x * x).sum(dim=-1, keepdim=True).clamp_min(eps)

    # Projection coefficient s: how much of f is parallel to x
    s = sm / sd  # [B, T, 1]

    # Parallel component: f_parallel = s * x
    f_parallel = s * x

    # Orthogonal component: f_perp = f - f_parallel
    f_perp = f - f_parallel

    # Optional interpolation between standard and orthogonal residuals
    if strength != 1.0:
        f_eff = (1.0 - strength) * f + strength * f_perp
    else:
        f_eff = f_perp
